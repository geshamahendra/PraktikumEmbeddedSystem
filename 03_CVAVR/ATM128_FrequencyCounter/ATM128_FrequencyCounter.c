...
start ^=1; //start= start XOR 1
lcd_gotoxy(0,1);
if (start==1)
{
  TCNT1H= 0x00; TCNT1L=0x00; //nilai awal Timer1
  TCNT3H= 0x00; TCNT3L=0x00; //nilai awal Counter3
  iterasiCounter3=0;         //nilai awal iterasi Counter 3
...
// Place your code here
 TCCR3B= 0x00;              //matikan Counter3
 TCCR1B= 0x00;               //matikan Timer1
 pulsa= (iterasiCounter3 * 50000) + ((long)TCNT3H * 256) + (long)TCNT3L; //jumlah pulsa dalam 50 ms
 pulsa= pulsa * 20;          //jumlah pulsa per detik
 lcd_gotoxy(4,1);
 sprintf(text, "%-8u Hz", pulsa);
 lcd_puts(text);
 
 //lalu kembali nyalakan timer dan counter
 TCNT1H= 0x00; TCNT1L=0x00; //nilai awal Timer1
 TCNT3H= 0x00; TCNT3L=0x00; //nilai awal Counter3 
 iterasiCounter3=0;         //nilai awal iterasi Counter 3
 TCCR1B= 0x0A;              //start Timer1
