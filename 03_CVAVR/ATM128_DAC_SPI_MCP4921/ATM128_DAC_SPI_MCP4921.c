...
{
 unsigned int temp;
 unsigned low_byte_data, high_byte_data;
 
 temp= MCP4921_Command;      // temp= xxxx xxxx 0111 0000
 temp= temp << 8;            // temp= 0111 0000 0000 0000
 temp= temp + data;          // temp= 0111 D11 D10 D9 D8....D0
 low_byte_data= temp & 0x00FF;//low_byte_data= D7 D6 D5.6....D0
 high_byte_data= temp >> 8;  // high_byte data= 0111 D11 D10 D9 D8
 LDAC= 1;
 DAC_CS= 1;
 DAC_CS= 0;           
 SPDR= high_byte_data;
...
// Characters/line: 16
lcd_init(16);
lcd_puts("Demo DAC MCP4921");

while (1)
      {
      // Place your code here
      for (data= 0; data<= 4095; data=data+25)
            {
              send2dacMCP4921(data);
              sprintf(text,"Data= %4d", data);
              lcd_gotoxy(0,1);
              lcd_puts(text);
              delay_ms(200);
            };
