...
#asm("sei")
putsf("Ketik 'g' untuk mulai atau lanjut, dan ketik yang lain untuk stop\r");

while (1)
      {
      // Place your code here
      
      if (rx_counter0 != 0) c= getchar();
      if (c=='g')       //tanda petik tunggal untuk input karakter
          {
           lcd_gotoxy(0,0);
           lcd_putsf("Transmisi Start");
           adc_data= read_adc(no_sensor);
           printf("%4d\r", adc_data);
           sprintf(text, "Data(%d)=%4d", no_sensor, adc_data);
           lcd_gotoxy(0,1);
           lcd_puts(text);
           delay_ms(100);
           no_sensor++;
           if (no_sensor>7) no_sensor=0; 
          } else
              { 
                lcd_gotoxy(0,0);
                lcd_putsf("Transmisi Stop ");
              };
      }
}
