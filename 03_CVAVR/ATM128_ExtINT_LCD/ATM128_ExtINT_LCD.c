...
void start_timer(void)
    {
        TCNT1H = 0x00;
        TCNT1L = 0x00;      //inisialisasi Timer1 = 0x0000
        iterasi = 0;
...
lcd_gotoxy(0,1);
sprintf(textbuffer,"Counter= %-5d", jumlah);
lcd_puts(textbuffer);
start_timer();
}
...
// Place your code here
stop_timer();
if (jumlah>0)jumlah--;//jumlah=jumlah-1
lcd_gotoxy(0,1);
sprintf(textbuffer,"Counter= %-5d", jumlah);
lcd_puts(textbuffer);
start_timer();
...
while (1)
      ...
      for (i=0;i<=15;i++) 
        {
        lcd_gotoxy(i,0);
        lcd_putchar(text_fis[i]);
        delay_ms(100);
        };
      for (i=15;i>=1;i--) 
        {
        lcd_gotoxy(i,0);
        lcd_putchar(20); // atau lcd_putchar(' ');
        delay_ms(100);
        };
      }
}
